...
graph TD
    MW["HTTP 框架中间件\nexpress / next / fastify / hono / fetch / axios\ntypescript/packages/http/"]
    ...
    FC["协调者 Facilitator\nx402Facilitator · httpFacilitatorClient\ntypescript/packages/core/src/facilitator/"]
    ME["区块链机制层\n@x402/evm · @x402/svm · @x402/stellar\ntypescript/packages/mechanisms/"]
    EX["扩展模块\nBazaar / EIP-2612 / SignInWithX\ntypescript/packages/extensions/"]
    MCP["MCP AI 集成\n@x402/mcp\ntypescript/packages/mcp/"]

    MCP -->|"AI Agent 触发支付请求"| CL
    CL -->|"生成 PAYMENT-SIGNATURE header"| MW
    MW -->|"HTTPAdapter 抹平框架差异"| RS
    RS -->|"PaymentPayload + Requirements"| FC
    FC -->|"verify / settle 链上操作"| ME
    CL -->|"createPaymentPayload 签名"| ME
    EX -->|"enrichPaymentRequired / enrichPaymentPayload"| RS
    EX -->|"enrichPaymentPayload 客户端扩展"| CL
